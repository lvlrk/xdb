...
std::vector<XdbEntry> Xdb::GetEntries() { return entries; }
...
XdbEntry::XdbEntry(const XdbEntry &oldEntry)
    : buffer(std::move(oldEntry.buffer)) { // phat copier
        stat = oldEntry.stat;
...
int Xdb::ReadFromFile(const std::string &filename) {
    ...
    for (int i = 0; i < entries.size(); i++) {
        ...
        if (i == 0)
            entries[i].filename = entries[i].filename.substr(4);
        ...
        inf.read(reinterpret_cast<char *>(entries[i].buffer.data()),
                entries[i].bufferSize);
    }
